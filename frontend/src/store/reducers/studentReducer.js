...
const studentSlice = createSlice({
  ...
  extraReducers: (builder) => {
    // Get Dashboard
    builder.addCase(student.getDashboard.pending, (state) => {
      state.status = "loading";
    });

    builder.addCase(student.getDashboard.fulfilled, (state, action) => {
      state.data = action.payload;
      state.status = "success";
    });

    builder.addCase(student.getDashboard.rejected, (state, action) => {
      state.status = "error";
      console.log("fetch Dashboard failed");
    });
...
export const { setStatusIdle } = studentSlice.actions;
